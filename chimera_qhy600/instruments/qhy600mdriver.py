...
class QHY600MDriver:
    ...
    def start_exposure(self, exptime):
        ...
        log.info(f"SetQHYCCDParam(CONTROL_EXPOSURE) - result: {result} | exptime: {self.exposure_time.value} us")

        self.qhyccd.SetQHYCCDBinMode(self.camera_handle, self.bin_factor, self.bin_factor)

        self.image_width = ctypes.c_uint32(self.image_width.value // self.bin_factor.value)
        self.image_height = ctypes.c_uint32(self.image_height.value // self.bin_factor.value)
    ...
    def start_readout(self, mode, top, left, width, height):
        ...
        length = self.image_width.value * self.image_height.value * (self.bpp.value // 8)
        ...
        log.info(f"GetQHYCCDSingleFrame() - result: {result} | width: {width.value} | height: {height.value} | bpp: {bpp.value} | channel: {channel.value}")
